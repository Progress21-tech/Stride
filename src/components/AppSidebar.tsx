...
export function AppSidebar() {
  ...
  return <aside className={`relative z-30 flex shrink-0 flex-col rounded-2xl border border-[var(--border-color)] bg-[var(--bg-surface)]/95 p-2 shadow-sm backdrop-blur-xl transition-[width] duration-200 md:sticky md:top-3 md:h-[calc(100vh-1.5rem)] ${expanded ? 'w-full md:w-60' : 'w-full md:w-14'}`}>
    ...
    <nav aria-label="Workspace navigation" className={`${expanded ? 'flex' : 'hidden md:flex'} mt-5 flex-1 flex-col gap-1`}>
    ...
    <div className={`${expanded ? 'block' : 'hidden md:block'} space-y-1 border-t border-[var(--border-color)] pt-2`}>
